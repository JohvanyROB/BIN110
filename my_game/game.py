...
class Game:
    ...
    def __init_player(self):
        side = randint(0, 1)
        pose = (self.env["height"]//2, 0) if side == 0 else (self.env["height"]//2, self.env["width"]-1)
    ...
    def process(self, move):
        if move == self.allowed_moves["move_left"]:
            self.__move_left()
        elif move == self.allowed_moves["move_right"]:
            self.__move_right()
        elif move == self.allowed_moves["move_up"]:
            self.__move_up()
        elif move == self.allowed_moves["move_down"]:
            self.__move_down()
        else:
            self.__do_nothing()
        self.__check_win()
