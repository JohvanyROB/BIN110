...
class GUI:
    def __init__(self, player_pose, env, player_name, fps=30, cell_size=50):
    ...
    def on_init(self):
        ...
        pygame.display.set_caption("BIN11O - Find the treasure") #titre de la fenetre de jeu
        ...
        self.player_name_text = Text(f"{self.player_name}", self.env["width"]*self.cell_size + self.right_panel_width//2 + self.offset, self.screen_res[1]//4, font_size=40, is_bold=True, color=BLUE)  #texte contenant le nom du joueur, il sera affiche dans la fenetre de jeu
        self.cpt_text = Text(f"Moves: {self.inc}", self.env["width"]*self.cell_size + self.right_panel_width//2 + self.offset, self.screen_res[1]*3//4, font_size=40, color=BLUE)   #texte contenant le nombre de coups realise par le personnage, il sera affiche dans la fenetre de jeu
        self.victory_text = Text("Congrats, you found the treasure!", self.screen_res[0]//2, self.screen_res[1]//2, color=GREEN, font_size=40, is_bold=True)    #texte contenant le message de victoire, il sera affiche en fin de partie si le joueur a gagne
    ...
    def draw(self):
        ...
        if self.mode == "party":    #si la partie n'est pas finie (le tresor n'a pas encore ete trouve et le personnage n'est pas rentre dans un mur)
            ...
            if self.player_state == "victory":  #si le joueur a gagne
                self.victory_text.set_content(f"Congrats, you found the treasure in {self.inc} moves!")
